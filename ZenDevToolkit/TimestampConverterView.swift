...
struct TimestampConverterView: View {
    ...
    enum ConversionMode: CaseIterable {
        ...
        var inputLabel: String {
            switch self {
            case .toHuman: return "Unix Timestamp"
            case .toTimestamp: return "Date & Time"
            }
        }

        var outputLabel: String {
            switch self {
            case .toHuman: return "Human-Readable Date"
            case .toTimestamp: return "Unix Timestamp"
            }
        }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 10) {
                    Text(conversionMode.inputLabel)
                        .font(.system(size: 11, weight: .medium))
                        .foregroundColor(.secondary)
                ...
                // Compact date picker (only in "To Timestamp" mode when toggled)
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 10) {
                    Text(conversionMode.outputLabel)
                        .font(.system(size: 11, weight: .medium))
                        .foregroundColor(.secondary)
